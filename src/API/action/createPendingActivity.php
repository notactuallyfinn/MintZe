...
try {
    $sqlRun = "INSERT INTO machtA(SID, Kuerzel, AID, bestaetigt, Klassenstufe, Stufe, Titel, Datum) VALUES ('$SID', '$kuerzel', '$AID', 0, '$gradeLevel', (SELECT Stufe FROM optionen WHERE AID = '$AID' AND ALID = '$ALID'), '$title', '$date')";
    $conn->query($sqlRun);
} catch (Exception $e) {
    throwError("Something went wrong with the query" . $SID . $AID . $date, 500, "Etwas ist mit der Datenbankabfrage falsch gelaufen");
    exit();
}
